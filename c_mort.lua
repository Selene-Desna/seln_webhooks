    ...
    while true do
        ...
        if IsEntityDead(playerPed) and not alreadyDead then
            ...
            if checkArray (Melee, death) then
                TriggerServerEvent('playerDied', playerName .. " - *Tué*")
            elseif checkArray (Bullet, death) then
                TriggerServerEvent('playerDied', playerName .. " - *Tué par balle*")
            elseif checkArray (Knife, death) then
                TriggerServerEvent('playerDied', playerName .. " - *Poignardé*")
            elseif checkArray (Car, death) then
                TriggerServerEvent('playerDied', playerName .. " - *Frappé à Mort*")
            elseif checkArray (Animal, death) then
                TriggerServerEvent('playerDied', playerName .. " - *Animal*")
            elseif checkArray (FallDamage, death) then
                TriggerServerEvent('playerDied', playerName .. " - *Chute*")
            elseif checkArray (Explosion, death) then
                TriggerServerEvent('playerDied', playerName .. " - *Explosion*")
            elseif checkArray (Gas, death) then
                TriggerServerEvent('playerDied', playerName .. " - *Intoxiqué*")
            elseif checkArray (Burn, death) then
                TriggerServerEvent('playerDied', playerName .. " - *Brulé Vif*")
            elseif checkArray (Drown, death) then
                TriggerServerEvent('playerDied', playerName .. " - *Noyade*")
            else
                TriggerServerEvent('playerDied', playerName .. " - *Cause Inconnue*")
            end
...
function LogDiscord(wbhk, name, message, color)
	...
	TriggerServerEvent('LogDiscordSRV', wbhk, name, message, color)
